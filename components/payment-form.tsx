...
export function PaymentForm({ action, payment, invoices, profiles }: { action: (data: FormData) => Promise<void>; payment?: Payment;
  ...
  return <Panel className="p-5"><form action={action} className="grid gap-4">
    ...
    <div className="grid gap-4 sm:grid-cols-2">
      <label className="text-sm">Invoice<select name="invoiceId" required defaultValue={payment?.invoiceId ?? ""} className={fieldClass}><option value="">Select invoice</option>{invoices.map(item => <option key={item.id} value={item.id}>{item.invoiceNumber}</option>)}</select></label>
