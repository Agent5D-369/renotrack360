...
async function main(){
 ...
 try{
  ...
  await post(route,{...await fields(route,'name="ownerReviewed"'),priceSnapshotId:price.id,sourceFileId:file.id,ownerReviewed:'on',scope:'Synthetic client scope in retained contract document.',exclusions:'Synthetic exclusions in retained contract document.',allowances:'Synthetic allowances in retained contract document.',schedule:'Synthetic schedule assumptions in retained contract document.',paymentSchedule:'Synthetic payment schedule in retained contract document.',warranty:'Synthetic warranty terms in retained contract document.',requiredDeposit:'100'});
  ...
  writeFileSync('.preservation/proposal-local-http.json',JSON.stringify({verifiedAt:new Date().toISOString(),ownerUploadAndIssuance:true,clientPublicContentOnly:true,exactPdf:true,clientApprovalRetry:true,ownerConversion:true,baselineEntry:true,anonymousStaffDenied:true,syntheticEstimateId:estimate.id,syntheticJobId:conversion.job.id},null,2));
  console.log('PASS: actual owner upload/issuance, public retained scope/PDF, client approval/retry, owner conversion and financial-review handoff.');
